  ...
  Redis redisClient = Redis.createClient(
    ...
    router.get("/pessoas/:id").respond(ctx -> {
      ...
      return redis.get(id).compose(valueInCache -> {
        ...
        }).onSuccess(jsonPerson -> redis.mset(List.of(id, jsonPerson.encode())));
      });
    });
    ...
    router.get("/pessoas")
      .respond(ctx -> {
        ...
        return redis.get(t).compose(valueInCache -> {
          if (valueInCache != null) {
          ...
          return selectDb.preparedQuery("SELECT id, apelido, nome, nascimento, stack FROM pessoa WHERE BUSCA_TRGM LIKE $1 LIMIT 50")
            ...
            .map(query -> {
              ...
              JsonArray personArray = new JsonArray(StreamSupport.stream(
                Spliterators.spliteratorUnknownSize(query.iterator(), Spliterator.ORDERED),
                false
              ).map(MainVerticle::buildPersonResponse).toList());

              return personArray;
            }).onSuccess(personArray ->  redis.mset(List.of(t,personArray.encode())));
    ...
    router.post("/pessoas").handler(ctx -> {
      ...
      //redis.mset(List.of(id, jsonPerson.encode()));
